...
from model.ConvNext_CTT_attention import MMF
from utils_attention import create_lr_scheduler, get_params_groups, train_one_epoch, evaluate, plot_average_roc_curve
from torchvision import datasets, models, transforms
from model.resnet import ResNetWithText
...
def main(args):
    ...
    for train_index, val_index in kf.split(dataset):
        ...
        for epoch in range(args.epochs):
            ...
            # validate
            val_loss, val_acc, val_recall, val_specificity, val_precision, val_f1, fold_roc_data = evaluate(model=model,
                                                                                             data_loader=val_loader,
                                                                                             device=device,
                                                                                             epoch=epoch,
                                                                                             criterion=criterion,
                                                                                             fold=fold,
                                                                                             save_dir=os.path.join(args.save_dir, "auc曲线")
                                                                                                            )

            total_roc_data.append(fold_roc_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_dir", type=str, default="E:\\研究生\\毕业\\工作点2")
